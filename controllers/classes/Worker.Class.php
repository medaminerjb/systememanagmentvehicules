    ...
    class Worker extends DataBase {
        ...
        public static function addWorker() {
            ...
            if($emailCount > 0){
                ...
                $add = "INSERT INTO users( fullName, bateOfBirth, phoneOne, phoneTwo, idNumber, userEmail, userUuid, userRole, userPassword) VALUES (:fullName, :DoB, :workerPhone1, :workerPhone2, :workerId, :workerEmail,  :userUuid, :userRole, :userPassword)";
                $addStmt = self::$pdo->prepare($add);
                $addStmt->execute([
               
                    ':fullName' => self::$fullName,
                    ':DoB' => self::$DoB,
                    ':workerPhone1' => self::$mobilePhone,
                    ':workerPhone2' => self::$workerPhone2,
                    ':workerId' => self::$workerId,
                    ':workerEmail' => self::$email,
        ...
        public static function tousWorkers(){
            $workers = "SELECT  userUuid, fullName, userEmail, userRole, userStatus  FROM users WHERE userRole != 'superAdmin' ";
            $driverCheckStmt = self::$pdo->prepare($workers);
        ...
        public static function getWorkers() {
            ...
            $workers = "SELECT userUuid, fullName, userEmail, userRole, userStatus FROM users WHERE userRole != 'superAdmin' ORDER BY id DESC LIMIT :s, :t";
        ...
        public static function updateWorker() {
            $add = "UPDATE users SET fullName = :fullName, bateOfBirth = :DoB, phoneOne = :workerPhone1, phoneTwo = :workerPhone2, idNumber = :workerId, userEmail = :workerEmail, userRole = :userRole WHERE userUuid = :userUuid";
            $addStmt = self::$pdo->prepare($add); 
            $addStmt->execute([
            ':fullName' =>self:: $fullName,
             ':DoB' =>self:: $DoB,
