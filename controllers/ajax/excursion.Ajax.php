    ...
    if (CheckUser::$userAvailable === true) {
        if ($_SESSION['userRole'] === 'admin' || $_SESSION['userRole'] === 'superAdmin' || $_SESSION['userRole'] === 'chauffeur' || $_SESSION['userRole'] === 'Agent') {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                ...
                if ($_GET['methode'] === 'allexcursions') {
                    Excursion::allexcurisons();
                   
                } else {
                    if(isset($_GET['search']) && trim($_GET['search']) !== '' && $_GET['search'] !== NULL){
                        Excursion::$currentPage = isset($_GET['page']) ? (int)$_GET['page'] : 1;
              
                        Excursion::searchExcursion($_GET['search']);
                          }
                else {   Excursion::$currentPage = isset($_GET['page']) ? (int)$_GET['page'] : 1;
                ...
                if ($_POST['method'] === 'POST') {
                    ...
                    Excursion::$ligne = $_POST['ligne'];
                    Excursion::$duree = $_POST['duree'];
